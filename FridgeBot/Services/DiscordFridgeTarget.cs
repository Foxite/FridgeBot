	...
	private Action<DiscordMessageBuilder> GetFridgeMessageBuilder(FridgeEntry fridgeEntry, SharpMessage message, SharpMessage? existingFridgeMessage, DiscordChatClient discord) {
		return dmb => {
			...
			if (message.ReferencedMessage != null) {
				if (message.ReferencedMessage.Author is SharpMember replyingToMember && !string.IsNullOrEmpty(replyingToMember.Nickname)) {
			...
			int i = 0;
			foreach ((SharpEmoji? emoji, _) in reactions) {
				if (i > 0) {
			...
			foreach ((SharpEmoji? emoji, int count) in reactions) {
			...
			} else if (message.Author is SharpMember authorMember) {
			...
			string description = message.MessageType switch {
				...
				MessageType.ApplicationCommand => $"{(message.Interaction.User is SharpMember interactionMember ? interactionMember.Nickname : message.Interaction.User.Username)} used ${message.Interaction.Name}",
				...
				MessageType.RecipientRemove => $"{authorName} removed {(message.MentionedUsers[0] as SharpMember)?.Nickname ?? message.MentionedUsers[0].Username} from the thread.",
				MessageType.AutoModAlert => $"AutoMod has blocked a message from {(message.Author as SharpMember)?.DisplayName ?? message.Author.Username}",
